...
public class FTBUltimineClient extends FTBUltimineCommon {
	...
	/*========== here ↓==========*/
	private static boolean canUseSetPacket = false;
	private static int targetIdx = -1;
	private static int latestIdx = -1;
	private static boolean isKeyShapelessPressed = false;
	...
	/*========== here ↓==========*/
	@Override
	public void setCanUseSetPacket(boolean b) {
		canUseSetPacket = b;
	}
	/*========== here ↑==========*/
	...
	public void clientTick(Minecraft mc) {
		...
		/*========== here ↓==========*/
		if ((pressed = keyBinding.isDown() || ShortcutKeyManager.isShortcutKeyPressed()) != p) {
		/*========== here ↑==========*/
			new KeyPressedPacket(pressed).sendToServer();
		}

		/*========== here ↓==========*/
		if (ShortcutKeyManager.isShortcutKeyPressed() || execute || isKeyShapelessPressed) {
			if (!execute) {
				if (ShortcutKeyManager.isShortcutKeyPressed() && !execute) {
					targetIdx = ShortcutKeyManager.getTargetShapeIdx(ShortcutKeyManager.getCurrentPressedShortcutKey());
					if (ShortcutKeyManager.Key_Shapeless.isDown() && !isKeyShapelessPressed) {
						isKeyShapelessPressed = true;
						latestIdx = shapeIdx;
					}
				} else if (isKeyShapelessPressed && !execute && targetIdx == shapeIdx) {
					isKeyShapelessPressed = false;
					targetIdx = latestIdx;
					latestIdx = -1;
				}
			}

			if (shapeIdx != targetIdx && (targetIdx >= 0 && targetIdx <= ShapeRegistry.shapeCount())) {
				if (canUseSetPacket) {
					new ModeSetPacket(targetIdx).sendToServer();
				} else {
					hasScrolled = true;
					boolean scrollDirection = (targetIdx - shapeIdx) > 0;
					if (Math.abs(targetIdx - shapeIdx) >= (ShapeRegistry.shapeCount() / 2))
						scrollDirection = !scrollDirection;
					new ModeChangedPacket(scrollDirection).sendToServer();
				}
			}

			execute = (shapeIdx != targetIdx && (targetIdx >= 0 && targetIdx <= ShapeRegistry.shapeCount()));
		}
		/*========== here ↑==========*/
